$(function() {
	...
	$(".zan_user").click(function(){
		...
		if(login == 1) {
			numzan++;
			if(numzan % 2 == 0) {
				$(this).css({"background-image":"url(img/article/ZAN-1.png)","color":"#999"});
				zongzan--;
				$(this).text(zongzan);
				...
				$(this).css({"background-image": "url(img/article/ZAN-2.png)","color":"#245fce"});
				zongzan++;
				$(this).text(zongzan);
				$(this).append("<div class='comment_on'>点赞成功<div>");
				setTimeout(function(){
	...
	$(".hf_user").click(function(){
		if(login==1){
			for (var i=0;i<$(".reply_cnt li").length;i++) {
				$(".reply_comments").remove();
				
			}
			var username=$(this).parent().prev().prev().find("span:first-child").html()
				var zishu1=$(".reply_hfnr textarea").keyup();
				$(".reply_cnt_user").css("height","200px");
				$(this).parent().parent().parent().css("height","305px");
				$(this).parent().parent()
				.append("<div class='reply_comments'>"+
							"<img src='img/article/HF-TX.png'>"+
							"<div class='reply_hfnr'>"+
							"<p class='hf_name'>回复：<span>"+username+"</span>:</p>"+
							...
							"<div>");
				$(".reply_hfnr>textarea").css("text-indent",$('.hf_name').width()+5+"px");
				
			
				$(".reply_hfnr textarea").keyup(function(){
					$(".user_ys").text($(".reply_hfnr>textarea").val().length);
					$(".user_sy").text(10000-$(".reply_hfnr>textarea").val().length)
...
//					}
			...
			$(".reply_button").click(function(){
				$(this).append("<div class='comment_on'>评论成功<div>");
				setTimeout(function(){
					$(".comment_on").remove();
					$(".reply_comments").remove();
					$(".reply_cnt>li").css("height","198px");
